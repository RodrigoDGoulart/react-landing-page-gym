...
const Membership = () => {
    return (
        <section className="w-full  h-auto py-20 md:py-28 lg:py-40 bg-zinc-950 flex flex-col md:gap-28 gap-20 justify-center items-center">
            <div className="flex flex-col items-center relative before:absolute before:-bottom-6 before:left-30 before:w-36 before:h-1 before:rounded-lg before:bg-gradient-to-r before:from-green-500 before:to-green-950 z-10">
                <Text as="p" className="text-green-500 md:text-sm text-xs tracking-widest uppercase">{MembershipPlans.firstText}</Text>
            ...
            <main className="grid lg:w-[90%] md:w-[96%] w-[90%] md:grid-cols-3 items-center gap-8 md:gap-4 lg:gap-8">
                {
                    MembershipPlans.cards.map((card, index) => (
                        <Card className={`w-full flex flex-col items-center gap-4 border border-zinc-500  transition-all duration-200 cursor-pointer hover:border-green-950/50 ${index === 1 ? "lg:py-16 py-10" : "py-10"}`} key={index}>
                            ...
                            <Text as="h3" className={`capitalize text-base font-semibold w-full py-2 text-center  text-zinc-100 my-3 ${card.caption.includes('12') ? "bg-gradient-to-r to-green-500 from-green-950" : "bg-zinc-800"}`}>{card.caption}</Text>
