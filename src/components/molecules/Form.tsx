    ...
    const calculateBmi = (event: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
        ...
        if (heightValue && weightValue) {
            ...
            if (Number(bmi) < 18.5) {
                ...
                message = 'Voce esta com obesidade.';
            }
        ...
        <main className="w-full grid gap-6 pb-10 md:pb-0">
            ...
            <InputField className="w-full flex flex-col gap-2">
                ...
                <div className="w-full relative">
                    <input type="number" placeholder="Digite sua altura" className="w-full h-12 pl-4 pr-8 text-zinc-400 outline-none transition-all duration-200 focus:border-green-500 border border-zinc-400 bg-transparent" value={heightValue} onChange={(event: React.ChangeEvent<HTMLInputElement>) => setHeightValue(event.target.value)} />
            ...
            <InputField className="w-full flex flex-col gap-2">
                ...
                <div className="w-full relative">
                    <input type="number" placeholder="Digite seu peso" className="w-full h-12 pl-4 pr-8 text-zinc-400 outline-none transition-all duration-200 focus:border-green-500 border border-zinc-400 bg-transparent" value={weightValue} onChange={(event: React.ChangeEvent<HTMLInputElement>) => setWeightValue(event.target.value)} />
                    <Text as="span" className="absolute text-zinc-200 font-bold top-3 right-4">kg</Text>
                </div>
            </InputField>

            <div className="w-full mt-4">
                <Button onClick={calculateBmi} type="button" className="px-6 py-3 bg-gradient-to-r from-green-950 to-green-500 text-zinc-200 text-sm uppercase font-semibold">Calcular</Button>
            </div>

            {
                bmiValue && <div className="w-full flex flex-col p-4 bg-zinc-700">
                    ...
                    <Text as="p" className="text-green-500">{bmiMessage}</Text>
                </div>
            }
